                ...
                <div class="col-lg-12">
                    ...
                    <div class="row respVacancy">
                        ...
                        <div class="col-lg-5">
                            <div class="applyBox">
                                <ul>
                                    <li><span>{{ $settings['elanin_vaxti'] }}</span><span>{{ $vacancy->created_at->format('Y-m-d') }}</span></li>
                                    <li><span>{{ $settings['staj'] }}</span><span>{{ $vacancy->duration}}</span></li>
                                    <li><span>{{ $settings['yas_heddi'] }}</span><span>{{ $vacancy->age_limit }}</span></li>
                                    <li><span>{{ $settings['tehsil'] }}</span><span>{{ $vacancy->education }}</span></li>
                                    <li><span>{{ $settings['is_rejimi'] }}</span><span>{{ $vacancy->work_schedule }}</span></li>
                                    <li><span>{{ $settings['muqavile'] }}</span><span>{{ $vacancy->contract_type }}</span></li>
                                    ...
                                    <form action="{{ route('cv.submit',['lang' => app()->getLocale()]) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <li>    <span>{{ $settings['cv_yukle'] }}</span>  <input type="file" name="cv_file" id="cv_file">@error('cv_file')
                                            <div class="alert alert-danger">{{ $message }}</div>
                                            @enderror</li>

                                        <button type="submit">{{ $settings['muraciet_et'] }}</button>
                                    </form>
                                </ul>
